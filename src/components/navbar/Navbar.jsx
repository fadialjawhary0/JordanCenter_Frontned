  ...
  return (
    <nav className={`${NAVBAR_STYLES.navbarBase} ${isRTL ? 'rtl' : ''}`}>
      <div className={`${NAVBAR_STYLES.navbarContainer} flex-wrap lg:flex-nowrap`}>
        ...
        <div className={`flex items-center gap-1.5 md:gap-2 lg:gap-4 shrink-0 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <Link to="/contact">
            ...
            >
                ...
                <motion.div
                  className={`absolute top-1/2 -translate-y-1/2 w-5 h-5 md:w-7 md:h-7 ${isWhiteBackgroundPage ? 'bg-white' : 'bg-[var(--color-inverse)]'} rounded-full flex items-center justify-center`}
                  initial={false}
                  animate={{
                    left: isContactHovered
                      ? isRTL
                        ? '6px'
                        : 'calc(100% - 34px)'
                      : isRTL
                ...
                >
                  <div className={`flex items-center ${isWhiteBackgroundPage ? 'text-[#020617]' : 'text-[var(--color-text)]'}`}>
                    ...
                    <ChevronRight
                      className={`w-3 h-3 md:w-4 md:h-4 -ml-1.5 md:-ml-2 ${isRTL ? 'rotate-180' : ''}`}
                    />
                  </div>
                </motion.div>
